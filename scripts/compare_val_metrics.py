...
def _print_latex_across_methods(names: List[str],
    ...
    linear_indices = [i for i, lf in enumerate(linear_flags) if lf]
    method_sets = [set(results_list[i].keys()) for i in non_linear_indices if results_list[i]]
    linear_method_sets = [set(results_list[i].keys()) for i in linear_indices if results_list[i]]
    methods = set()
    if method_sets:
        methods |= (set.intersection(*method_sets) if only_intersection else set.union(*method_sets))
    if linear_method_sets:
        for s in linear_method_sets:
            methods |= s
    ...
    # Rows per method reflect all encoders
    visible_encoders = [(i, nm) for i, nm in enumerate(names)]
